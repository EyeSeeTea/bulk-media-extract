import { LocalDirectoryGateway } from "$/application/storage/LocalDirectoryGateway";
...
export class BrowserLocalDirectoryGateway implements LocalDirectoryGateway {
    ...
    public async selectDirectory(): Promise<FileSystemDirectoryHandle> {
        if (!this.supportsDirectorySelection()) {
            throw new Error(
    ...
    public async validateDirectoryAccess(handle?: FileSystemDirectoryHandle): Promise<void> {
        if (!this.supportsDirectorySelection()) {
            throw new Error(
                "Local directory export is not supported in this browser. Use a Chromium-based browser or switch to WebDAV."
            );
    ...
    public async writeResponse(params: {
