...
Failures to create a subdirectory, open a writable file, or write file content MUST be reported against the affected operation while preserving already completed files in the selected directory. The application execution flow MUST treat the local-directory writer as an interchangeable storage-side collaborator so this reporting behavior stays consistent with other storage methods.
